...
const DonationIntentHandler = {
    canHandle(handlerInput) {
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
            && Alexa.getIntentName(handlerInput.requestEnvelope) === 'DonationIntent';
    },
    handle(handlerInput){
        let speakOutput = "Hi!";
        let responseBuilder = handlerInput.responseBuilder;
...
const PROGRAMS_TOKEN =  'ProgramsToken';



const ProgramsIntentHandler = {
    canHandle(handlerInput) {
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
            && Alexa.getIntentName(handlerInput.requestEnvelope) === 'ProgramsIntent';
    },
    handle(handlerInput){
        let speakOutput = "Hi!";
        let responseBuilder = handlerInput.responseBuilder;
        if (Alexa.getSupportedInterfaces(handlerInput.requestEnvelope)['Alexa.Presentation.APL']){
            ...
            responseBuilder.addDirective({
                type: 'Alexa.Presentation.APL.RenderDocument',
                token: PROGRAMS_TOKEN,
                document: programsDucument,
            });
            
            // Tailor the speech for a device with a screen.
            speakOutput += " Here is Unos institute programs page"
        } else {
            speakOutput += " This example would be more interesting on a device with a screen, such as an Echo Show or Fire TV."
        }
        return responseBuilder
...
/* 
    ...
    Intents: "I want to know more", know more.

 */

const KNOW_MORE_TOKEN =  'ProgramsToken';
...
const KnowMoreIntentHandler = {
    canHandle(handlerInput) {
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
            && Alexa.getIntentName(handlerInput.requestEnvelope) === 'KnowMoreIntent';
    },
    handle(handlerInput){
        let speakOutput = "Hi!";
        let responseBuilder = handlerInput.responseBuilder;
        if (Alexa.getSupportedInterfaces(handlerInput.requestEnvelope)['Alexa.Presentation.APL']){
            ...
            responseBuilder.addDirective({
                type: 'Alexa.Presentation.APL.RenderDocument',
                token: KNOW_MORE_TOKEN,
                document: knowMoreDocument,
            });
            
            // Tailor the speech for a device with a screen.
            speakOutput += "Here is more information about us!"
        } else {
            speakOutput += " This example would be more interesting on a device with a screen, such as an Echo Show or Fire TV."
        }
        return responseBuilder
